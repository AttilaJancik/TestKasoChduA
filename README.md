# TestKasoChduA

A short example of communication with Kverkom for the Kaso CHDU device

## Features

- initialize device with tester parameters
- communication with tax office server
- console informations of input and output data

## Requirements

installed .NET 4.7.2 SDK or later or .NET Core 3.1 SDK or later
installed Kaso Technologies CHDU software (version 2.0.9.1 or later)

## Installation

clone this project and compile for use

## Usage

1. set actual data for variables: taxisKverkomSettings , taxisDrvAPI.Init , taxisQrKodPlatbyParams
2. if need tracking of server, set variables: taxisZrusQrKodPlatbyParams , taxisOverQrKodPlatbyParams , taxisInfoQrKoduPlatbyParams
3. set variable **runFunction** for run selected function
4. push *Start without debugging* in IDE (VS-26)

## Author

- *Created by Attila Jancik*
- [Email](mailto:attila@gecora.com)
- [Website](https://gecora.com)
- [GitHub](https://github.com/AttilaJancik)
- LinkedIn

## Acknowledgements

Thanks to Kaso Technologies for their CHDU software and support.
Thanks to the open-source community for their contributions and inspiration.

## Disclaimer

This software is provided "as is", without warranty of any kind, express or implied.
In no event shall the authors be liable for any claim, damages or other liability, whether in an action of contract, tort or otherwise, arising from, out of or in connection with the software or the use or other dealings in the software.
Use at your own risk.
The author is not responsible for any damage caused by the use of this software.
Make sure to comply with all applicable laws and regulations when using this software.
Always back up your data before using this software.
This software is intended for educational and informational purposes only.
The author does not guarantee the accuracy or reliability of the software.
Users are responsible for their own actions when using this software.
The author reserves the right to modify or discontinue the software at any time.
By using this software, you agree to the terms of this disclaimer.
If you do not agree with these terms, do not use the software.
Thank you for understanding.
Use responsibly!
Happy coding!
Enjoy using myCHDUcom!
Good luck!
Attila Jančik
Gecora.com
https://gecora.com

## Contributing

Contributions are welcome! Please fork the repository and submit a pull request.
Report bugs and suggest features via the GitHub issues page.
Please follow the code of conduct when contributing to this project.
Make sure to write clear and concise commit messages.
Include tests for any new features or bug fixes.
Document your code thoroughly.
Respect the project's coding style and conventions.
Engage with the community in a positive and constructive manner.
Thank you for your contributions!
Together we can make myCHDUcom even better!

## Contact
- For questions or support, please contact Attila Jančik at attila@gecora.com
- Visit the project page on GitHub:

---

## Note
for testing is need device Kaso CHDU